...
public class NewsArticleService : INewsArticleService
{
    ...
    public async Task<string> UpdateAtricleContentAsync(NewsArticleUpdateContentDto updateDto)
    {
        if (updateDto == null)
            throw new ArgumentNullException(nameof(updateDto));

        if (string.IsNullOrEmpty(updateDto.Id))
            throw new ArgumentNullException(nameof(updateDto.Id));
        ...
        var foundArticle = await _articleRepository.GetAsNoTrackingAsync(updateDto.Id) ?? 
            throw new InvalidOperationException($"Article not found with id: {updateDto.Id}");
        ...
        return await _articleRepository.UpdateAsync(foundArticle)
            ?? throw new InvalidOperationException($"Failed to update content for article with id: {updateDto.Id}");
    }
}
